...
export function getHelloPanelHtml(): string {
    ...
    <body>
        <div class="container">
            ...
            <button onclick="navigator('searchAndJump')">Search-And-Jump</button>
            <button onclick="navigator('showSearchHistory')">Show-Search-History</button>
        </div>
        
        <script>
            const vscode = acquireVsCodeApi();
            function navigator(cmd, message='From -helloPanel'){
              vscode.postMessage({
                  command: cmd,
                  msg: message
              });
